/*Jiang 2016.1.30 check rating difference of a trustor and trustee*/
/*need to sort records by items*/
...
public class itemuser2 {
	...
	public void readTrust()
	 ...
	 public int checkItemExist(String ItemID)
	   {
		   int flag=-1; //default: not exist
		   for(int i=0;i<itemList.size();i++)
		   {
			   if(itemList.get(i).ItemID.equals(ItemID))
			   {
				   flag=i;
				   break;
			   }
	                ...
	                while ((s = br.readLine()) != null) {
	                  ...
	                  // if(this.checkItemNodeDuplication(nodeList.get(o),item)) //if already exist, go to next
	                   ...
	                   {//if(itemList.get(itemList.size()-1).ItemID.equals(edge[1])) //same item with last one
		                   // already exist this item the itemList
	                	   int iFlag =this.checkItemExist(edge[1]); 
	                	   if(iFlag!=-1)
		                   { 
		                	   itemList.get(iFlag).nodes.add(nodeList.get(o)); 
